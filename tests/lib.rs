...
use math_render::math_box::{MathBox, MathBoxContent};
use math_render::Glyph;
...
fn assume_boxes<'a, 'b, T>(content: MathBoxContent<&'a [MathBox<'b, T>], Glyph>)
                           -> &'a [MathBox<'b, T>] {
    match content {
        MathBoxContent::Boxes(list) => list,
        _ => panic!(),
    }
}
...
fn fraction_centering_test() {
    TEST_FONT.with(|font| {
        ...
        let width = fraction_bar.width();

        // test if the numerator is centered
        let num = &boxes[0];
        let left_space = num.origin.x - left_edge;
        let right_space = width - num.width() - left_space;
        ...
        let right_space = width - denom.width() - left_space;
        println!("(left, right) = {:?}", (left_space, right_space));
        // allow rounding errors
        assert!((left_space - right_space).abs() <= 2);
    })
