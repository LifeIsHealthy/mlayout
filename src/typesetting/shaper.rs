...
pub struct HarfbuzzShaper<'a> {
    ...
    math_table: Option<Shared<Blob<'a>>>,
...
impl<'a> HarfbuzzShaper<'a> {
    pub fn new(font: Shared<Font>) -> HarfbuzzShaper {
        ...
        HarfbuzzShaper {
            font: font.clone(),
            math_table: font.face().table_with_tag(b"MATH"),
            buffer: buffer,
        }
    }
    ...
    fn shape_with_style(&self, string: &str, style: LayoutStyle) -> GlyphBuffer {
        ...
        if style.flat_accent {
            features.push(Feature::new(Tag::new('f', 'l', 'a', 'c'), 1, ..));
        }
...
fn try_assembly<'a>(
    ...
    let result = assembly_iter
        ...
        .enumerate()
        .map(move |(index, (glyph, advance))| {
            if index >= (part_count - 1) as usize {
                ShapedGlyph {
                    advance: point_with_offset(advance, horizontal),
                    ..glyph
                }
